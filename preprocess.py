...
from fairseq.tokenizer import tokenize_line, tokenize_smiles
...
def main(args):
    ...
    def build_dictionary(filenames, src=False, tgt=False, fileformat='smiles'):
        assert src ^ tgt
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        merge_result(
            Binarizer.binarize(
                ...
                tokenize=tk, offset=0, end=offsets[1]
            )
        )
        if num_workers > 1:
            pool.join()
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, append_eos=True):
    ...
    res = Binarizer.binarize(
        ...
        tokenize=tk, append_eos=append_eos, offset=offset, end=end
    )
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
...
def binarize_alignments(args, filename, parse_alignment, output_prefix, offset, end):
    ...
    res = Binarizer.binarize_alignments(
        filename, parse_alignment, consumer, 
        tokenize=tk, offset=offset, end=end
    )
    ds.finalize(dataset_dest_file(args, output_prefix, None, "idx"))
    return res
